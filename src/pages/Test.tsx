...
export default function Test() {
  ...
  async function getTodayEarned(uid: string): Promise<number> {
    try {
      ...
      debugLog('Test', 'getTodayEarned: 結果', { uid, sum, records: snaps.docs.length })
      return sum
    } catch (e) { 
      debugLog('Test', 'getTodayEarned: 錯誤', { uid, error: e })
      return 0 
    }
  }
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        try {
          type FetchArgs = { lang: string; grade?: number; limit?: number; after?: string; signal?: AbortSignal; startPage?: number; pageCount?: number }
          // 智能加载策略：先加载当前游标附近的数据，后台加载其余数据
          const cursorKey = `typesprout_cursor_${lang || 'en-US'}`
          const cur0 = Number(localStorage.getItem(cursorKey) || '0')
          // 從 index.json 讀取實際 pageSize 和總頁數
          let pageSize = 500
          let totalPages = 15
          try {
            const res = await fetch('/index.json', { signal: controller.signal })
            const idx = await res.json() as { pageSize?: number; filesPerLang?: Record<string, number> }
            if (idx && Number(idx.pageSize)) pageSize = Math.max(1, Number(idx.pageSize))
            if (idx && idx.filesPerLang && idx.filesPerLang[lang || 'en-US']) {
              totalPages = Number(idx.filesPerLang[lang || 'en-US'])
            }
          } catch {
            // 使用默認值
          }
          // 计算当前游标所在的页面
          const currentPage = Math.max(1, Math.floor(cur0 / pageSize) + 1)
          const pagesToLoad = Math.min(3, totalPages) // 先加载3页数据，确保有足够题目
          const startPage = Math.max(1, currentPage - 1) // 从当前页的前一页开始
          
          // 先加载当前游标附近的数据（快速响应）
          items = await (fetchContent as (p: FetchArgs) => Promise<SheetItem[] | null>)({ 
            lang: (lang || 'en-US'), 
          ...
          // 后台加载其余数据（不阻塞UI）
          if (totalPages > pagesToLoad) {
            setTimeout(() => {
              const remainingPages = totalPages - pagesToLoad
              const nextStartPage = startPage + pagesToLoad
              const fetchContentTyped = fetchContent as (p: { lang: string; grade?: number; limit?: number; after?: string; signal?: AbortSignal; startPage?: number; pageCount?: number }) => Promise<SheetItem[] | null>
              fetchContentTyped({ 
                lang: (lang || 'en-US'), 
                limit: pageSize * remainingPages, 
                startPage: nextStartPage, 
                pageCount: remainingPages, 
                signal: undefined 
              }).then((moreItems: SheetItem[] | null) => {
                if (moreItems && moreItems.length > 0) {
                  // 将额外数据合并到缓存中
                  try {
                    const cacheKey = `typesprout_qs_cache_${lang || 'en-US'}`
                    const cachedArr = JSON.parse(localStorage.getItem(cacheKey) || '[]') as string[]
                    const newItems = moreItems.map((item: SheetItem) => {
                      const st = item.structured as StructuredText | undefined
                      return String(st?.q || item.text || '')
                    })
                    const merged = [...cachedArr, ...newItems]
                    localStorage.setItem(cacheKey, JSON.stringify(merged.slice(0, 800)))
                  } catch {
                    // 忽略缓存错误
                  }
                }
        ...
        // 游標：同語言持久化；智能重置機制確保用戶每次都能從新的開始
        const cursorKey = `typesprout_cursor_${lang || 'en-US'}`
        let cur0 = Number(localStorage.getItem(cursorKey) || '0')
        const total = pool.length
        
        // 优先从Firebase读取用户进度（如果已登录）
        if (user) {
          try {
            const ref = doc(db, 'profiles', user.uid)
            const snap = await getDoc(ref)
            const pdata = snap.data() as Record<string, unknown> | undefined
            if (pdata?.lastCursorLang === (lang || 'en-US') && typeof pdata?.lastCursor === 'number') {
              cur0 = pdata.lastCursor
              console.log(`从Firebase恢复进度：游标位置 ${cur0}`)
            }
          } catch (e) {
            if (import.meta.env.DEV) console.warn('read Firebase cursor failed', e)
          }
        }
        
        // 智能游标重置：如果已看过的题目超过总题目的80%，重置游标
        const seenRatio = unionSeen.size / Math.max(1, total)
        const shouldReset = seenRatio > 0.8 // 80%的题目已看过
        ...
        if (shouldReset) {
          ...
          if (user) {
            try {
              ...
              if (import.meta.env.DEV) console.warn('reset Firebase progress failed', e)
            }
          }
          console.log(`题库重置：已看过${Math.round(seenRatio * 100)}%的题目，重置游标到开始`)
        } else {
          // 正常游标逻辑
          startIdx = Number.isFinite(cur0) ? Math.max(0, Math.min(cur0, Math.max(0,total-1))) : 0
        }
  ...
  const submitCurrentQuestion = useCallback(() => {
    ...
    console.log(`Test: 第${qIndex + 1}题提交 - 题目长度: ${text.length}, 输入长度: ${input.length}, 累加前totalTypedRef: ${totalTypedRef.current}`)
    totalTypedRef.current += input.length
    console.log(`Test: 第${qIndex + 1}题提交 - 累加后totalTypedRef: ${totalTypedRef.current}`)
  ...
  useEffect(() => {
    ...
    if (remain <= THRESH && !prefetchingRef.current) {
      ...
      try {
        ...
        ;(fetchContent as (p: { lang: string; limit?: number; startPage?: number; pageCount?: number; signal?: AbortSignal }) => Promise<SheetItem[] | null>)({ lang: L, limit: 200, startPage: nextStart, pageCount: 1, signal: undefined })
          .then((more: SheetItem[] | null) => {
            if (!more || !more.length) return
            try {
              const cacheKey = `typesprout_qs_cache_${L}`
              const cachedArr = JSON.parse(localStorage.getItem(cacheKey) || '[]') as string[]
              const merge = [...cachedArr, ...more.map((m: SheetItem)=> String((m.structured as StructuredText)?.q || m.text || ''))]
              localStorage.setItem(cacheKey, JSON.stringify(merge.slice(0, 1000)))
            } catch (e) {
            console.warn('Failed to load page size from index.json:', e)
          }
        ...
        <div className="mt-1">
          <div className="relative">
            ...
            <Button disabled={!readyToStart} onClick={() => { if (!readyToStart) return; startAtRef.current = Date.now(); setRunning(true); // 真正開始時才落盤游標與已看過集合
                  ...
                  try { if (user) void setDoc(doc(db,'profiles',user.uid), { 
                    lastSeenIds: mergedSeenIdsRef.current.slice(-1000),
                    lastCursor: newCursorRef.current,
                    lastCursorLang: lang || 'en-US'
                  }, { merge: true }) } catch (e) { if (import.meta.env.DEV) console.warn('persist profile progress failed', e) }
